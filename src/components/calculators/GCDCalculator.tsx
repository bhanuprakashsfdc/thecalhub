...
function GCDCalculator() {
  const [nums, setNums] = useState('48, 18, 60');
  ...
  return (
    <div className="bg-surface-container-low p-8 rounded-xl border border-white/5">
      <div className="flex items-center gap-2 text-primary-fixed mb-6"><Calculator className="w-4 h-4" /><span className="text-[10px] uppercase tracking-[0.2em] font-bold">Greatest Common Divisor</span></div>
      <div className="group mb-6"><label className="block text-[10px] uppercase tracking-[0.2em] text-neutral-500 font-bold mb-3">Numbers (comma separated)</label><input className="w-full bg-surface-container-highest border-none rounded-lg py-4 px-4 text-white mono text-xl" type="text" value={nums} onChange={(e) => setNums(e.target.value)} placeholder="48, 18, 60" /></div>
      <div className="bg-surface-container-highest p-6 rounded-xl"><p className="text-neutral-500 text-xs">GCD</p><p className="text-3xl font-bold text-white mono">{result}</p></div>
    </div>
  );
}
