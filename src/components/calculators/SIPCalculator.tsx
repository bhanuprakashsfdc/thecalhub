...
export default function SIPCalculator() {
  ...
  const calculate = () => {
    ...
    setResult({ invested, interest, maturity: futureValue });
  };
    ...
    <div className="mt-14 p-6 md:p-10 flex-1 overflow-y-auto">
      <div className="mb-12">
        ...
        <p className="text-neutral-400 max-w-xl text-lg leading-relaxed">
          Calculate returns on your Systematic Investment Plan with compound interest.
        </p>
      </div>
      ...
      <div className="max-w-lg">
        <div className="bg-surface-container-low rounded-xl p-6 border border-white/5">
          <div className="space-y-6">
            <div>
              <label className="block text-[10px] font-mono text-neutral-500 uppercase tracking-widest mb-2">Monthly Investment</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[10px] font-mono text-neutral-500 uppercase tracking-widest mb-2">Expected Return (%)</label>
                <input
                  type="number"
            ...
            {result && (
              ...
              >
                <div className="flex items-center gap-2 text-primary-fixed mb-4">
                  ...
                  <span className="text-[10px] uppercase tracking-widest font-bold">SIP Returns</span>
                </div>
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <span className="text-neutral-400 text-sm">Amount Invested</span>
                    <span className="text-lg font-bold text-white">₹{result.invested.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-neutral-400 text-sm">Interest Earned</span>
                    <span className="text-lg font-bold text-primary-fixed">₹{result.interest.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                  </div>
                  <div className="pt-3 border-t border-white/10 flex justify-between items-center">
                    <span className="text-neutral-400 text-sm">Maturity Value</span>
                    <span className="text-2xl font-extrabold text-primary-fixed">₹{result.maturity.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                  </div>
                </div>
              </motion.div>
            )}
