...
function CombinationCalc() {
  const [n, setN] = useState(5);
  const [r, setR] = useState(3);
  
  const factorial = (x: number): number => x <= 1 ? 1 : x * factorial(x - 1);
  const combination = n >= r && r >= 0 ? factorial(n) / (factorial(r) * factorial(n - r)) : 0;
  
  return (
    <div className="bg-surface-container-low p-8 rounded-xl border border-white/5">
      <div className="flex items-center gap-2 text-primary-fixed mb-6"><Calculator className="w-4 h-4" /><span className="text-[10px] uppercase tracking-[0.2em] font-bold">Combination: C(n,r) = n!/(r!(n-r)!)</span></div>
      ...
      <div className="bg-surface-container-highest p-6 rounded-xl"><p className="text-neutral-500 text-xs">Combinations</p><p className="text-3xl font-bold text-white mono">{combination.toFixed(0)}</p></div>
    </div>
  );
}
export default CombinationCalc;