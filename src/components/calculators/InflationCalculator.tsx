  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div className="lg:col-span-7"><div className="bg-surface-container-low p-8 rounded-xl border border-white/5 shadow-2xl">
        ...
        <div className="bg-surface-container-highest p-8 rounded-xl"><p className="text-4xl font-bold text-white mono">${Math.abs(inflationLoss).toFixed(2)}</p><p className="text-neutral-500 text-sm mt-2">Loss due to inflation</p></div>
        <div className="mt-6 bg-surface-container-highest p-6 rounded-xl"><p className="text-neutral-500 text-xs">Future Value in {years} years: ${futureValue.toFixed(2)}</p><p className="text-neutral-500 text-xs mt-2">Purchasing Power Lost: ${Math.abs(inflationLoss).toFixed(2)}</p></div>
      </div></div>
    </div>
  );
}
